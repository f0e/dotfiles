...
# Send uncommitted changes to another machine
gitsend() {
  ...
  echo "metadata filename: metadata-$TIMESTAMP.json"
  if ! echo "$metadata" | tailscale file cp --name "metadata-$TIMESTAMP.json" - "$machine_name:"; then
    echo "failed to send metadata to $machine_name" >&2
    return 1
  fi
  echo "metadata sent successfully"
  
  echo "sending patch to $machine_name..."
  if _create_patch | tailscale file cp --name "changes-$TIMESTAMP.patch" - "$machine_name:"; then
    echo "✓ changes sent to $machine_name"
    echo "  repository: $REPO_NAME"
    echo "  commit: $COMMIT_HASH"
    echo "  files: changes-$TIMESTAMP.patch, metadata-$TIMESTAMP.json"
  else
...
_find_latest_metadata() {
  local recv_dir="$1"
  # Simple approach: just find any metadata file and use the most recent by name
  ls -t "$recv_dir"/metadata-*.json 2>/dev/null | head -1
}
...
_validate_patch() {
  ...
  # Check patch age (15 minutes = 900 seconds)
  ...
  # Set global flag for backup creation
...
_apply_patch() {
  ...
  # Create temporary files for staged and unstaged patches
  local temp_staged temp_unstaged
  temp_staged=$(mktemp)
  temp_unstaged=$(mktemp)
  
  # Split patch file into staged and unstaged parts
  awk '
...
# Receive and apply uncommitted changes
gitrecv() {
  _validate_git_sync || return 1
  _get_repo_info
  ...
  # First, check if files were auto-downloaded to Downloads
  ...
  if [[ -d "$downloads_dir" ]]; then
    ...
    recent_patches=($(find "$downloads_dir" -name "changes-*.patch" -mtime -1 2>/dev/null | sort -r))
    
    if [[ ${#recent_patches[@]} -gt 0 ]]; then
      echo "found patch files in Downloads, moving to receive directory..."
      # Move patch files and any corresponding metadata
      for patch_file in "${recent_patches[@]}"; do
        local base_name=$(basename "$patch_file")
        local timestamp="${base_name#changes-}"
        timestamp="${timestamp%.patch}"
        
        mv "$patch_file" "$recv_dir/" 2>/dev/null
        # Also move metadata file if it exists
        if [[ -f "$downloads_dir/metadata-$timestamp.json" ]]; then
          mv "$downloads_dir/metadata-$timestamp.json" "$recv_dir/" 2>/dev/null
        fi
      done
  ...
  # If no files found in Downloads, try the inbox
  if [[ "$found_in_downloads" == false ]]; then
    ...
    if ! tailscale file get --verbose "$recv_dir" >/dev/null 2>&1; then
      ...
      # If inbox is empty, wait for new files
  ...
  # Find and validate metadata file
  local metadata_file
  ...
  if [[ ! -f "$metadata_file" ]]; then
    ...
    find "$recv_dir" -name "*.json" -type f 2>/dev/null || echo "no .json files found"
    echo ""
    echo "trying to find patch files and extract info from filename..."
    
    # Try to find patch files and extract timestamp from filename
    local patch_files
    patch_files=($(find "$recv_dir" -name "changes-*.patch" -type f 2>/dev/null | sort -r))
    
    if [[ ${#patch_files[@]} -eq 0 ]]; then
      echo "no patch files found either" >&2
      return 1
    fi
    
    # Use the most recent patch file
    local latest_patch="${patch_files[0]}"
    local filename=$(basename "$latest_patch")
    local timestamp="${filename#changes-}"
    timestamp="${timestamp%.patch}"
    
    echo "found patch file: $filename"
    echo "extracted timestamp: $timestamp"
         echo "proceeding without metadata (some validation will be skipped)..."
     
     # Create minimal metadata - try to extract info from patch file
     repo_name="unknown"
     commit_hash="unknown" 
     created_at="unknown"
     sender_machine="unknown"
     has_staged="true"
     has_unstaged="true"
     
     # Try to extract repo and commit info from patch file header
     if [[ -f "$latest_patch" ]]; then
       local patch_header
       patch_header=$(head -5 "$latest_patch")
       if [[ "$patch_header" =~ "Git patch for ([^ ]+) at commit ([^ ]+)" ]]; then
         repo_name="${match[1]}"
         commit_hash="${match[2]}"
       fi
       if [[ "$patch_header" =~ "Machine: ([^ ]+)" ]]; then
         sender_machine="${match[1]}"
       fi
     fi
     
     local patch_file="$latest_patch"
     
     echo ""
     echo "patch information (from file):"
     echo "  repository: $repo_name"
     echo "  from commit: $commit_hash"
     echo "  sender: $sender_machine"
     echo "  timestamp: $timestamp"
     
     # Skip validation since we don't have metadata
     CREATE_BACKUP=false
  else
    ...
    # Validate patch compatibility (sets CREATE_BACKUP global)
    _validate_patch "$current_repo" "$current_commit" || return 1
    
    # Find and apply patch
    local patch_file="$recv_dir/changes-$timestamp.patch"
  fi
  ...
  if _apply_patch "$patch_file"; then
    ...
    # Remove backup stash if everything went well
...
# Get online Tailscale machines for completion
